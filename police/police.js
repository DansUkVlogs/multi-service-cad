...
// --- LOGGING HELPERS FOR USER ACTIONS (POLICE PAGE) ---
...
document.addEventListener('DOMContentLoaded', () => {
    // Do NOT set sessionStorage.unitId automatically from callsign display on page load.
    // Only set sessionStorage.unitId after valid login/setup.
    const unitIdInit = sessionStorage.getItem('unitId');
    console.log('[FORCE LOGOUT INIT] Police page unitId:', unitIdInit);
    // Always initialize force logout if user is logged in
    if (unitIdInit && unitIdInit !== 'None' && unitIdInit !== 'SELECT CALLSIGN') {
        initializeForceLogout(db);
    } else {
        // If not logged in, ensure force logout modal is removed/hidden
        const modal = document.getElementById('force-logout-modal');
        if (modal) modal.remove();
    }

    // Initialize messaging system for police page
    const policeUser = {
        type: 'police',
        id: sessionStorage.getItem('unitId') || 'police-unit',
        name: sessionStorage.getItem('callsign') || 'Police Unit',
        canSendMessages: false, // Police units typically only receive messages from admin
        canReceiveMessages: true
    };
    initializeMessaging(policeUser);
    ...
    // --- BROADCAST LISTENER & UI (Police PAGE) ---
    let callsign = null;
    ...
    if (window._policeAttachListenerActive) return;
    window._policeAttachListenerActive = true;
                    ...
                    try {
                        if (callIdForDetach && unitId) {
                            ...
                            if (callClosingDoc.exists()) {
                                ...
                                const closingData = callClosingDoc.data();
                        ...
                        // If Firestore fails, default to showing popup for regular detaches
                        console.warn('Error checking call closing status:', e);
                        shouldShowPopup = true;
                    }
                }
...
// (Merged into single DOMContentLoaded handler above)
    ...
    if (newCallBtn && newCallModal && closeNewCallModalBtn && newCallForm) {
        ...
        newCallBtn.addEventListener('click', () => {
            ...
            // Reset form fields
            newCallForm.reset();
            document.getElementById('callerName').value = 'POLICE DISPATCH';
        });
        // Close modal
...
async function saveDetails() {
    ...
    try {
        ...
        if (unitId && unitId !== "None") {
            ...
            // User just logged in, remove force logout modal if present
            const modal = document.getElementById('force-logout-modal');
            if (modal) modal.remove();
            // Reinitialize messaging with the real unitId and callsign
            try {
                const policeUserReal = {
                    type: 'police',
                    id: unitId,
                    name: callsignInput,
                ...
                initializeMessaging(policeUserReal);
            } catch (e) {
                console.warn('[POLICE] Failed to reinitialize messaging with real unitId', e);
            }
            // Initialize force logout system now that unitId exists
            try { initializeForceLogout(db); } catch (e) { console.warn('[POLICE] initializeForceLogout failed', e); }
        }
        if (civilianId && civilianId !== "None") {
        ...
        // If the unit has a linked civilian, reinitialize messaging to listen for civilian messages too
        try {
            if (civilianId && civilianId !== 'None') {
                cleanupMessaging();
                const policeUserWithExtras = {
                    type: 'police',
                    id: unitId,
                    name: callsignInput,
                    canSendMessages: false,
                ...
                initializeMessaging(policeUserWithExtras);
            }
        } catch (e) {
            console.warn('[POLICE] Failed to reinitialize messaging with civilian extraId', e);
        }
...
function showBaseModal(onConfirm, onCancel) {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="modal-content">
                <h2 style="margin:0 0 10px 0;font-size:1.5rem;color:#004d00;text-align:center;font-weight:bold;">Go to Base</h2>
                <label for="base-location-select" style="font-weight:600;color:#333;margin-bottom:6px;margin-top:6px;font-size:1rem;">Select Police Station:</label>
    ...
    select.innerHTML = '';
    loadPoliceStationLocations().then(locations => {
        select.innerHTML = '<option value="" disabled selected>--Select Police Station--</option>';
        locations.forEach(loc => {
            const opt = document.createElement("option");
            opt.value = loc;
            opt.textContent = loc;
    ...
    function confirmHandler() {
        const location = select.value;
        if (!location) {
            showNotification("Please select a police station.", "error");
            return;
        }
        cleanup();
        // Clear other locations
...
// Listen for calls changes (Police & Multiple)
const callsRef = collection(db, 'calls');
const q = query(callsRef, where('service', 'in', ['Police', 'Multiple']));
...
async function renderAttachedUnitsForSelectedCall(callId, container) {
    ...
    try {
        ...
        for (const docSnap of attachedUnitSnapshot.docs) {
            ...
            abbr.style.background = getUnitTypeColor(unitData.unitType || 'Police');
            abbr.style.color = getContrastingTextColor(getUnitTypeColor(unitData.unitType || 'Police'));
...
// Function to display calls in the Police interface
    ...
    if (newCallForm) {
        newCallForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const newCall = {
                    callerName: callerName || 'POLICE DISPATCH',
...
function showUserIdentityModal() {
    ...
    modal.innerHTML = `
        <div style="background: white; padding: 30px; border-radius: 12px; min-width: 400px; box-shadow: 0 4px 20px rgba(0,0,0,0.3);">
            <h2 style="margin: 0 0 20px 0; color: #1565c0; text-align: center;">👤 User Identity</h2>
            ...
            <div style="display: flex; gap: 15px; justify-content: center;">
                <button id="save-identity-btn" 
                        style="background: #1565c0; color: white; border: none; padding: 12px 24px; border-radius: 6px; font-weight: bold; cursor: pointer;">
                    💾 Save
                </button>
    ...
    onSnapshot(callsRef, (snapshot) => {
        ...
        if (newCalls.length > 0) {
            ...
            if (hasPanicCall) {
                ...
                if (!dispatcherActive) {
                    playSoundByKey('newcall');
                    console.log('[SOUND] New regular call detected - playing new call sound (no dispatcher active)');
                } else {
